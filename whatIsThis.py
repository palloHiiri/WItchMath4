...
def linear_approximation(x, y):
    ...
    try:
        ...
        raise Exception('Коэффициентики не нашлись(')
...
def quadratic_approximation(x, y):
    ...
    sx = sum(x)
    s2x = sum(xi ** 2 for xi in x)
    s3x = sum(xi ** 3 for xi in x)
    s4x = sum(xi ** 4 for xi in x)
    sy = sum(y)
    sxy = sum(xi * yi for xi, yi in zip(x, y))
    s2xy = sum(xi * xi * yi for xi, yi in zip(x, y))
...
def cubic_approximation(x, y):
    ...
    s3x = sum(xi ** 3 for xi in x)
    s4x = sum(xi ** 4 for xi in x)
    s5x = sum(xi ** 5 for xi in x)
    s6x = sum(xi ** 6 for xi in x)
    sy = sum(y)
    sxy = sum(xi * yi for xi, yi in zip(x, y))
    s2xy = sum(xi * xi * yi for xi, yi in zip(x, y))
    s3xy = sum(xi * xi * xi * yi for xi, yi in zip(x, y))
...
def exponential_approximation(x, y):
    ...
    log_y = np.log(y)
    phi_linear, (a, b) = linear_approximation(x, log_y)
    b_exp = np.exp(b)
    phi = lambda x_: b_exp * np.exp(a * x_)
    return phi, (a, b_exp)
...
def logarithmic_approximation(x, y):
    ...
    log_x = np.log(x)
    phi_linear, (a, b) = linear_approximation(log_x, y)
...
def power_approximation(x, y):
    ...
    log_x = np.log(x)
    log_y = np.log(y)
    phi_linear, (b, ln_a) = linear_approximation(log_x, log_y)
    a = np.exp(ln_a)
...
def correlation_coefficient(x, y):
    ...
    sxy = sum(xi * yi for xi, yi in zip(x, y))
    s2x = sum(xi ** 2 for xi in x)
    s2y = sum(yi ** 2 for yi in y)

    numerator = n * sxy - sx * sy
    denominator = np.sqrt((n * s2x - sx ** 2) * (n * s2y - sy ** 2))
